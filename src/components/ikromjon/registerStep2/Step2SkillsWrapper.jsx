...
const Step2SkillsWrapper = () => {
  ...
  const addSkill = () => setFormData(prev => ({ ...prev, skills: [...prev.skills, { name: '', exp: '' }] }));
  const removeSkill = (index) => setFormData(prev => ({ ...prev, skills: prev.skills.filter((_, i) => i !== index) }));
  const addLanguage = () => setFormData(prev => ({ ...prev, languages: [...prev.languages, { name: '', level: '' }] }));
  ...
  const validateStep = () => {
    ...
    const filledSkills = formData.skills.filter(s => s.name && s.name.trim() !== '');
